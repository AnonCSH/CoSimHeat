...
int mm_read_unsymmetric_sparse(const char *fname, int *M_, int *N_, int *nz_, float **val_, int **I_, int **J_)
{
	FILE *f;
	MM_typecode matcode;
	int M, N, nz;
...
int mm_read_mtx_crd_data(FILE *f, int M, int N, int nz, int I[], int J[], float val[], MM_typecode matcode)
{
	int i;
	if (mm_is_complex(matcode))
	{
		for (i = 0; i < nz; i++)
			if (fscanf(f, "%d %d %f %f", &I[i], &J[i], &val[2 * i], &val[2 * i + 1]) != 4)
				return MM_PREMATURE_EOF;
	}
...
int mm_read_mtx_crd_entry(FILE *f, int *I, int *J, float *real, float *imag, MM_typecode matcode)
{
	if (mm_is_complex(matcode))
	{
		if (fscanf(f, "%d %d %f %f", I, J, real, imag) != 4)
			return MM_PREMATURE_EOF;
	}
	else if (mm_is_real(matcode))
	{
...
int mm_read_mtx_crd(char *fname, int *M, int *N, int *nz, int **I, int **J, float **val, MM_typecode *matcode)
{
	int ret_code;
	FILE *f;
